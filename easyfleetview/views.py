...
class Dashboard(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        today = datetime.now()
        last_fuel = FuelEntry.objects.order_by(
            'vehicle_id', '-date').distinct('vehicle_id')
        context['recent_fuel'] = FuelEntry.objects.filter(
            id__in=last_fuel).order_by('vehicle')
        context['upcoming_service'] = ServiceRecord.objects.filter(
            completed=False).order_by('vehicle')
        context['recent_service'] = ServiceRecord.objects.filter(
            completed=True).order_by('-date')[:5]
        ...
        ytd_fuel_total = FuelEntry.objects.filter(
            date__year=today.year).aggregate(Sum('cost'))
...
def stripe_webhooks(request):
    ...
    if event.type == 'payment_method.attached':
        ...
        BillingProfile.objects.filter(
            stripe_id=customer).update(paid_until=next_payment)
        ...
        next_payment = date.fromtimestamp(sub.current_period_end)
        BillingProfile.objects.filter(stripe_id=customer).update(
            paid_until=next_payment, canceled=sub.cancel_at_period_end)
        ...
        next_payment = date.fromtimestamp(
            stripe_subscription.current_period_end)
        BillingProfile.objects.filter(stripe_id=customer).update(
            email=update.email, paid_until=next_payment)
